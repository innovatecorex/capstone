...
@section('title', 'Edit Mapping — ' . $mapping->subject?->subject_name)
@section('breadcrumb', 'Edit Mapping')
...
<div class="enc-page__header">
  <div class="enc-page__title-row">
    <div>
      <h1 class="enc-page__title">Edit Mapping</h1>
      <p class="enc-page__subtitle">
        {{ $mapping->grade_level }} · {{ $mapping->subject?->subject_name }}
        @if($mapping->academicYear) · {{ $mapping->academicYear->year_label }} @endif
      </p>
    </div>
...
<form action="{{ route('admin.curriculum-mappings.update', $mapping) }}" method="POST">
@csrf @method('PUT')
<div class="cm-form-card">
  ...
  <div class="cm-form-section">
    ...
    <div class="cm-field">
      ...
      <select name="academic_year_id" class="cm-ctrl {{ $errors->has('academic_year_id') ? 'is-err' : '' }}" required>
        ...
        <option value="{{ $year->id }}" {{ old('academic_year_id', $mapping->academic_year_id) == $year->id ? 'selected' : '' }}>{{ $year->year_label }}</option>
        @endforeach
      </select>
    ...
    <div class="cm-grid-2">
      <div class="cm-field">
        ...
        <select name="grade_level" class="cm-ctrl {{ $errors->has('grade_level') ? 'is-err' : '' }}" required>
          ...
          <option value="{{ $level }}" {{ old('grade_level', $mapping->grade_level) === $level ? 'selected' : '' }}>{{ $level }}</option>
          @endforeach
        </select>
  ...
  @php $hasPrereq = !is_null(old('prerequisite_subject_id', $mapping->prerequisite_subject_id)); @endphp
  <div class="cm-form-section">
    <div class="cm-section-title"><span>2</span> Prerequisite Configuration</div>

    <label class="cm-prereq-toggle">
      <input type="checkbox" id="prereq-toggle" {{ $hasPrereq ? 'checked' : '' }}
    ...
    <div id="prereq-block" class="cm-prereq-block {{ $hasPrereq ? 'visible' : '' }}">
      <div class="cm-field">
        ...
        <select name="prerequisite_subject_id" id="prereq-subject" class="cm-ctrl">
          ...
          @foreach($subjects as $subject)
          @if($subject->id != $mapping->subject_id)
          <option value="{{ $subject->id }}"
            {{ old('prerequisite_subject_id', $mapping->prerequisite_subject_id) == $subject->id ? 'selected' : '' }}>
            {{ $subject->subject_code }} – {{ $subject->subject_name }}
          </option>
          @endif
          @endforeach
      ...
      <div class="cm-field" style="max-width:200px;">
        ...
        <input type="number" name="prerequisite_min_grade" id="prereq-grade"
               value="{{ old('prerequisite_min_grade', $mapping->prerequisite_min_grade ?? 75) }}"
  ...
  <div class="cm-form-section">
    ...
    <div class="cm-grid-2">
      <div class="cm-field">
        ...
        <input type="number" name="sequence_order" value="{{ old('sequence_order', $mapping->sequence_order) }}"
               min="0" class="cm-ctrl">
  ...
  {{-- Actions --}}
  <div style="padding:1.1rem 1.5rem;background:#f8fafc;display:flex;gap:.65rem;align-items:center;">
    <button type="submit" class="enc-btn enc-btn--primary">Save Changes</button>
    ...
    <form method="POST" action="{{ route('admin.curriculum-mappings.destroy', $mapping) }}"
          style="margin-left:auto;" onsubmit="return confirm('Delete this mapping? This cannot be undone.')">
      @csrf @method('DELETE')
      <button type="submit" style="font-size:.82rem;color:#dc2626;font-weight:700;background:none;border:none;cursor:pointer;padding:.5rem 0;">
        Delete Mapping
      </button>
    </form>
  </div>

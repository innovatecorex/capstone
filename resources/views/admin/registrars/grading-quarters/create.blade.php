@extends('layouts.app')
@section('title', 'Add Grading Period')
@section('breadcrumb', 'Add Grading Period')
...
<div class="enc-page__header">
  <div class="enc-page__title-row">
    <div>
      <h1 class="enc-page__title">Add Grading Period</h1>
      <p class="enc-page__subtitle">Create a grading period (quarter or semester) within an academic year.</p>
    </div>
...
<div class="enc-card" style="max-width:720px;">
  <form method="POST" action="{{ route('admin.grading-quarters.store') }}">
    @csrf
      ...
      <div>
        ...
        <select name="academic_year_id" required style="width:100%;padding:10px 12px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;font-size:.9rem;">
          ...
          @foreach($academicYears as $yr)
            <option value="{{ $yr->id }}" {{ old('academic_year_id', $academicYear?->id) == $yr->id ? 'selected' : '' }}>{{ $yr->year_label }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 2fr;gap:18px;">
        <div>
          ...
          <select name="quarter_number" required style="width:100%;padding:10px 12px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;font-size:.9rem;">
            @for($i = 1; $i <= 4; $i++)
              <option value="{{ $i }}" {{ old('quarter_number') == $i ? 'selected' : '' }}>{{ $i }}</option>
            @endfor
          </select>
        </div>
        <div>
          ...
          <input type="text" name="quarter_name" required maxlength="100" placeholder="e.g. 1st Quarter or 1st Semester"
    ...
    <div style="padding:16px 24px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;justify-content:flex-end;gap:10px;">
      ...
      <button type="submit" style="padding:.6rem 1.4rem;border:none;border-radius:8px;background:#1d4ed8;color:#fff;font-size:.875rem;font-weight:700;cursor:pointer;">Create Period</button>
    </div>
  </form>
</div>
